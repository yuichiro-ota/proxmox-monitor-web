  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal" role="dialog" aria-modal="true">
      ...
      <div class="modal-head">
        <Icon :name="headIcon" :size="20" class="modal-icon" />
      ...
      <div v-if="badges.length" class="modal-badges">
        <span v-for="b in badges" :key="b.text" class="badge" :class="`badge-${b.tone}`">{{ b.text }}</span>
      </div>

      <div v-if="bars.length" class="modal-metrics">
        <UsageBar
          v-for="m in bars"
          :key="m.label"
          :label="m.label"
      ...
      <div v-if="storages.length" class="modal-section">
      ...
      <div v-if="guestLines.length" class="modal-section">
...
.modal-sub { font-size: 0.78rem; color: var(--text-muted); font-variant-numeric: tabular-nums; }
...
.modal-metrics { display: flex; flex-direction: column; gap: 10px; }
